...
<?php
  ...
  require ("../config/db.php");
    ...
    $selectEnd = "SELECT sedes.idSede, sedes.nome, enderecos.cep FROM sedes INNER JOIN enderecos ON enderecos.idEndereco = sedes.FK_enderecos ORDER BY sedes.nome";
    $idEndereco = "idSede";
    $CEP= "cep";
    $Nome = "nome";
          ...
          <div class="">
            ...
            <select class="form-control" name="idEndereco2" id="destino">
              ...
              <?php while ($result = mysqli_fetch_array($RSelectEnd2)) {?>
                <option value="<?php echo $result[$idEndereco].'&'.$result[$CEP]?>"><?php  echo $result[$Nome]?></option>
              <?php }?>
            </select><br>
          </div>
            ...
            <a class="my-4">Endereço da Sede:</a><br>
              <select class="form-control" name="idSede" id="remetente">
                      ...
                      <?php while ($result = mysqli_fetch_array($RSelectSedes)) {?>
                        <option value="<?php echo $result[$idSede].'&'.$result[$CEPSede]?>"><?php echo $result[$NomeSede]?></option>
                      <?php }?>
              </select>
          </div><br>
